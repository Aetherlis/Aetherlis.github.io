var cursoreffects = function(t) {
    ...
    return t.bubbleCursor = function(t) {
        ...
        function y() {
            e.remove(), cancelAnimationFrame(i), s.removeEventListener("mousemove", g), s.removeEventListener("touchmove", m), s.removeEventListener("touchstart", m), window.removeEventListener("resize", A)
        }
        ...
        function b() {
            s.remove(), cancelAnimationFrame(c), n.removeEventListener("mousemove", C), n.removeEventListener("touchmove", M), n.removeEventListener("touchstart", M), window.removeEventListener("resize", E)
        }
        ...
        function J() {
            ...
            for (let t = 0; t < z; t++) R[t] = 0, Y[t] = 0, W[t] = 0, H[t] = 0;
        ...
        function k() {
            ! function() {
                W[0] = Math.round(R[0] += (l.y - R[0]) * m), H[0] = Math.round(Y[0] += (l.x - Y[0]) * m);
                for (let t = 1; t < z; t++) W[t] = Math.round(R[t] += (W[t - 1] - R[t]) * m), H[t] = Math.round(Y[t] += (H[t - 1] - Y[t]) * m), R[t - 1] >= c - 80 && (R[t - 1] = c - 80), Y[t - 1] >= h - 80 && (Y[t - 1] = h - 80)
            }(),
            function() {
                n.clearRect(0, 0, h, c);
                const t = new Date,
                ...
                for (let t = 0; t < F.length; t++) F[t].y = R[t] + 67.5 * Math.sin(-i + t * b * Math.PI / 180), F[t].x = Y[t] + 67.5 * Math.cos(-i + t * b * Math.PI / 180), n.fillStyle = F[t].color, n.fillText(F[t].value, F[t].x, F[t].y);
                for (let t = 0; t < T.length; t++) T[t].y = R[F.length + t] + 45 * Math.sin(t * B * Math.PI / 180), T[t].x = Y[F.length + t] + 45 * Math.cos(t * B * Math.PI / 180), n.fillStyle = T[t].color, n.fillText(T[t].value, T[t].x, T[t].y);
                for (let t = 0; t < D.length; t++) D[t].y = R[F.length + E + t] + 0 + t * S * Math.sin(a), D[t].x = Y[F.length + E + t] + 0 + t * S * Math.cos(a), n.fillStyle = D[t].color, n.fillText(D[t].value, D[t].x, D[t].y);
                for (let t = 0; t < X.length; t++) X[t].y = R[F.length + E + D.length + t] + 0 + t * S * Math.sin(s), X[t].x = Y[F.length + E + D.length + t] + 0 + t * S * Math.cos(s), n.fillStyle = X[t].color, n.fillText(X[t].value, X[t].x, X[t].y);
                for (let t = 0; t < I.length; t++) I[t].y = R[F.length + E + D.length + X.length + t] + 0 + t * S * Math.sin(i), I[t].x = Y[F.length + E + D.length + X.length + t] + 0 + t * S * Math.cos(i), n.fillStyle = I[t].color, n.fillText(I[t].value, I[t].x, I[t].y)
            }(), i = requestAnimationFrame(k)
        }

        function N() {
            e.remove(), cancelAnimationFrame(i), s.removeEventListener("mousemove", Z), s.removeEventListener("touchmove", Q), s.removeEventListener("touchstart", Q), window.removeEventListener("resize", U)
        }
        return P.onchange = () => {
            P.matches ? N() : J()
        }, J(), {
        ...
        function x() {
            d.remove(), cancelAnimationFrame(A), i.removeEventListener("mousemove", y), i.removeEventListener("touchmove", p), i.removeEventListener("touchstart", p), window.removeEventListener("resize", f)
        }
        ...
        function y(t, e, n) {
            l.push(new x(t, e, n)) // push new fairy dust (x, y, color)
        }
        ...
        function w() {
            r.remove(), cancelAnimationFrame(u), i.removeEventListener("mousemove", p), i.removeEventListener("touchmove", f), i.removeEventListener("touchstart", f), window.removeEventListener("resize", g)
        }
        ...
        function g() {
            e.remove(), cancelAnimationFrame(m), o.removeEventListener("mousemove", A), window.removeEventListener("resize", u)
        }
        return r.onchange = () => {
            r.matches ? g() : d()
        }, d(), {
        ...
        function f() {
            ...
            c = document.createElement("canvas"), l = c.getContext("2d"), c.style.top = "0px", c.style.left = "0px", c.style.pointerEvents = "none", e ? (c.style.position = "absolute", n.appendChild(c), c.width = n.clientWidth, c.height = n.clientHeight) : (c.style.position = "fixed", document.body.appendChild(c), c.width = r, c.height = d), n.addEventListener("mousemove", E), n.addEventListener("touchmove", y, {
                passive: !0
            }), n.addEventListener("touchstart", y, {
                passive: !0
            }), window.addEventListener("resize", p), C()
        }

        function p(t) {
            r = window.innerWidth, d = window.innerHeight, e ? (c.width = n.clientWidth, c.height = n.clientHeight) : (c.width = r, c.height = d)
        }

        function y(t) {
            if (t.touches.length > 0)
                for (let e = 0; e < t.touches.length; e++) M(t.touches[e].clientX, t.touches[e].clientY, m)
        }
        g.onchange = () => {
            g.matches ? L() : f()
        };
        ...
        function L() {
            c.remove(), cancelAnimationFrame(a), n.removeEventListener("mousemove", E), n.removeEventListener("touchmove", y), n.removeEventListener("touchstart", y), window.removeEventListener("resize", p)
        }
        ...
        function v() {
            e.remove(), cancelAnimationFrame(i), s.removeEventListener("mousemove", p), window.removeEventListener("resize", f)
        }
        ...
        function v() {
            e.remove(), cancelAnimationFrame(i), s.removeEventListener("mousemove", f), s.removeEventListener("touchmove", g), s.removeEventListener("touchstart", g), window.removeEventListener("resize", m)
        }
        ...
        function y() {
            e.remove(), cancelAnimationFrame(i), c.removeEventListener("mousemove", f), c.removeEventListener("touchmove", g), c.removeEventListener("touchstart", g), window.removeEventListener("resize", m)
        }
        ...
        function M() {
            e.remove(), cancelAnimationFrame(i), h.removeEventListener("mousemove", x), window.removeEventListener("resize", w)
        }
        return y.onchange = () => {
            y.matches ? M() : v()
        }, v(), {
        ...
        function w() {
            e.remove(), cancelAnimationFrame(i), s.removeEventListener("mousemove", y), window.removeEventListener("resize", p)
        }
